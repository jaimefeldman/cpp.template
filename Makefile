...
CCFLAGS := -std=c++17 -Wall -O3 -Wno-c++11-extensions 
CCFLAGS  := -std=c++17 -Wno-c++11-extensions -ggdb3
CFLAGS   := -std=c99 -Wall -pedantic
DEGUGFL  := -std=c++17 -Wall -Wno-c++11-extensions -ggdb
INCLUDES := src/clases/
CC       := g++
C		 := gcc
MKDIR    := mkdir -p
...
$(APP): $(OBJSUBDIRS) $(OBJECTS_FILES) $(C_OBJECTS_FILES)
	...
	if [ -d "bin" ]; then \
		$(CC) -o $(APP) $(OBJECTS_FILES) $(C_OBJECTS_FILES) $(CCFLAGS); \
	else \
		mkdir -p bin; \
		$(CC) -o $(APP) $(OBJECTS_FILES) $(CCFLAGS) $(C_OBJECTS_FILES); \
	fi
